...
queues:12=@1q queues:34=@2q queues:910=@3q queues:1011=@4q -- \
...
--id=@newqos create QoS type=linux-htb \
other-config:max-rate=2000000 \
queues:34=@1q queues:1011=@2q queues:56=@3q -- \
--id=@1q create queue other-config:min-rate=100000 other-config:max-rate=700000 -- \
--id=@2q create queue other-config:min-rate=100000 other-config:max-rate=300000 -- \
...
sudo ovs-ofctl add-flow s1 ip,priority=65500,nw_src=10.0.0.1,idle_timeout=0,actions=set_queue:1011,output:2
sudo ovs-ofctl add-flow s1 ip,priority=65500,nw_src=10.0.0.12,idle_timeout=0,actions=set_queue:1011,output:6
...
sudo ovs-ofctl add-flow s6 ip,priority=65500,nw_src=10.0.0.12,idle_timeout=0,actions=set_queue:1011,output:2
sudo ovs-ofctl add-flow s6 ip,priority=65500,nw_src=10.0.0.11,idle_timeout=0,actions=set_queue:1011,output:6
